...
- アンインストール時は適用前のファイルをバックアップから復元する
- 同じパスに別のファイルが置かれていた場合は上書きせず、バックアップを保持する
...
インストール状態は `~/.local/state/work-dotfiles/install-backup` に記録されます。バックアップから復元したファイル以外や、空になったバックアップディレクトリは自動削除されません。
